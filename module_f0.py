...
def downsample1d_block(
    ...
    h1_norm = instance_norm_layer(inputs = h1, activation_fn = activation_fuc, name = name_prefix + 'h1_norm')
    h1_gates = conv1d_layer(inputs = inputs, filters = filters, kernel_size = kernel_size, strides = strides, activation = activation_fuc, name = name_prefix + 'h1_gates')
    h1_norm_gates = instance_norm_layer(inputs = h1_gates, activation_fn = tf.nn.leaky_relu, name = name_prefix + 'h1_norm_gates')
    h1_glu = gated_linear_layer(inputs = h1_norm, gates = h1_norm_gates, name = name_prefix + 'h1_glu')
...
def generator_gatedcnn(inputs, reuse = False, scope_name = 'generator_gatedcnn'):
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        # Output
        o1 = conv1d_layer(inputs = u2, filters = 10, kernel_size = 15, strides = 1, activation = activation_fuc, name = 'o1_conv')
        o2 = tf.transpose(o1, perm = [0, 2, 1], name = 'output_transpose')

    return o2
    

def _discriminator(inputs, reuse = False, scope_name = 'discriminator'):
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        h1 = conv2d_layer(inputs = inputs, filters = 16, kernel_size = [3, 3], strides = [1, 2], activation = activation_fuc, name = 'h1_conv')
        h1_gates = conv2d_layer(inputs = inputs, filters = 16, kernel_size = [3, 3], strides = [1, 2], activation = activation_fuc, name = 'h1_conv_gates')
        h1_glu = gated_linear_layer(inputs = h1, gates = h1_gates, name = 'h1_glu')

        d1 = downsample2d_block(inputs = h1_glu, filters = 32, kernel_size = [3, 3], strides = [2, 2], name_prefix = 'downsample2d_block1_')
        d2 = downsample2d_block(inputs = d1, filters = 32, kernel_size = [3, 3], strides = [2, 2], name_prefix = 'downsample2d_block2_')
        d3 = downsample2d_block(inputs = d2, filters = 64, kernel_size = [6, 3], strides = [1, 2], name_prefix = 'downsample2d_block3_')
...
def discriminator(inputs, reuse = False, scope_name = 'discriminator'):
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        h1 = conv1d_layer(inputs = inputs, filters = 64, kernel_size = 3, strides = 1, activation = tf.nn.leaky_relu, name = 'h1_conv')
        h1_gates = conv1d_layer(inputs = inputs, filters = 64, kernel_size = 3, strides = 1, activation = tf.nn.leaky_relu, name = 'h1_conv_gates')
        h1_glu = gated_linear_layer(inputs = h1, gates = h1_gates, name = 'h1_glu')

        # Downsample
        d1 = downsample1d_block(inputs = h1_glu, filters = 128, kernel_size = 3, strides = 2, name_prefix = 'downsample2d_block1_')
        d2 = downsample1d_block(inputs = d1, filters = 256, kernel_size = 3, strides = 2, name_prefix = 'downsample2d_block2_')
        d3 = downsample1d_block(inputs = d2, filters = 512, kernel_size = 6, strides = 2, name_prefix = 'downsample2d_block3_')
