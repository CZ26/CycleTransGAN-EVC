...
def wavs_to_specs(wavs, n_fft = 2048, hop_length = None):
...
def wavs_to_mfccs(wavs, sr, n_fft = 2048, hop_length = None, n_mels = 128, n_mfcc = 24):
...
def sample_train_data(dataset_A, dataset_B, n_frames = 256):
...
#         print('frames_A_total', frames_A_total)
#         assert frames_A_total >= n_frames
        if frames_A_total <= n_frames:
            data_A = list(data_A)
            for di in range(len(data_A)):
                d = list(data_A[di])
                while len(d)<n_frames:
                    d.append(0.)
                data_A[di] = np.asarray(d)
        ...
        if frames_B_total <= n_frames:
            data_B = list(data_B)
            for di in range(len(data_B)):
                d = list(data_B[di])
                while len(d)<n_frames:
                    d.append(0.)
                data_B[di] = np.asarray(d)
